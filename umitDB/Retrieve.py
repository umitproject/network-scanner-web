...
class RawRetrieve:
    ...
    def get_service_info_id_from_db(self, info, service_name_id):
        ...
        s_id = self.cursor.execute("SELECT pk FROM service_info WHERE \
    ...
    def get_port_id_from_db(self, portid, fk_service_info, fk_protocol,
        ...
        p_id = self.cursor.execute("SELECT pk FROM port WHERE portid = ? \
                        AND fk_service_info = ? AND fk_protocol = ? AND \
                        fk_port_state = ?", (portid, fk_service_info, 
                                             fk_protocol, 
    ...
    def get_hostname_id_from_db(self, hostname):
        ...
        h_id = self.cursor.execute("SELECT pk FROM hostname WHERE \
    ...
    def get_address_id_from_db(self, address, a_type, vendor):
        ...
        a_id = self.cursor.execute("SELECT pk FROM address WHERE \
                            address = ? AND type = ? AND fk_vendor = ?",
                           (address, a_type, vendor)).fetchone()
    ...
    def get_vendor_id_from_db(self, name):
        ...
        v_id = self.cursor.execute("SELECT pk FROM vendor WHERE \
                        name = ?", (name, )).fetchone()
    ...
    def get_host_state_id_from_db(self, state):
        ...
        h_id = self.cursor.execute("SELECT pk FROM host_state WHERE \
    ...
    def get_tcp_sequence_id_from_db(self, tcpseq_dict):
        ...
        t_id = self.cursor.execute("SELECT pk FROM tcp_sequence WHERE \
    ...
    def get_tcp_ts_sequence_id_from_db(self, tcptsseq_dict):
        ...
        t_id = self.cursor.execute("SELECT pk FROM tcp_ts_sequence WHERE \
    ...
    def get_ip_id_sequence_id_from_db(self, ipidseq_dict):
        ...
        t_id = self.cursor.execute("SELECT pk FROM ip_id_sequence WHERE \
    ...
    def get_scanner_id_from_db(self, name, version):
        ...
        s_id = self.cursor.execute("SELECT pk FROM scanner WHERE \
...
class CompositeRetrieve(RawRetrieve):
    ...
    def get_ipv4_for_host_from_db(self, fk_host):
        ...
        """
        debug("Getting ipv4 address for host id %d.." % fk_host)
        ...
        for fk in fk_address:
            address = self.cursor.execute("SELECT address FROM address WHERE \
                                       type = 'ipv4' AND pk = ?", 
                                       (fk[0], )).fetchone()
            if address:
                break
    ...
    def get_scan_details_for_scan_from_db(self, scan):
        ...
        details = self.cursor.execute("SELECT args, xmloutputversion, verbose,\
                   debugging, scanner.name, scanner.version FROM scan \
                   JOIN scanner ON (scan.fk_scanner = scanner.pk) \
...
table port.." % host)
        ...
        pdata = self.cursor.execute("SELECT portid, fk_service_info, \
                                    fk_protocol, fk_port_state FROM port \
                                    JOIN _host_port ON \
                                    (_host_port.fk_port=port.pk) \
                                    WHERE _host_port.fk_host=?",
                                    (host, )).fetchall()
    ...
    def get_port_data_for_pdata_from_db(self, protocol_id, port_state_id,
        ...
        fullpdata = self.cursor.execute("SELECT protocol.name as protocol, \
                        ...
                        service_info.method, service_info.conf, \
                        service_name.name FROM protocol, port_state, \
                        service_info, service_name WHERE protocol.pk = ? AND \
                        port_state.pk = ? AND service_info.pk = ? AND \
                        service_name.pk = service_info.fk_service_name",
                        (protocol_id, port_state_id, 
                         service_info_id)).fetchall()[0]
    ...
    def get_extraports_data_for_host_from_db(self, host_id):
        ...
        epdata = self.cursor.execute("SELECT extraports.count, \
                            ...
                            port_state.pk = extraports.fk_port_state",
                            (host_id, )).fetchall()
...
class InventoryRetrieve(ConnectDB, CompositeRetrieve):
    """
    ...
    def get_hosts_id_for_scan_from_db(self, fk_scan):
        ...
        ids = self.cursor.execute("SELECT pk FROM host WHERE fk_scan = ? \
                                   AND fk_scan IN (SELECT fk_scan FROM \
                                   _inventory_scan)",
                                  (fk_scan, )).fetchall()
    ...
    def get_hosts_base_data_for_inventory_from_db(self, host_address, 
        ...
        """
        debug("Getting pks in host with host_address %s and fk_inventory %d" % \
              (host_address, fk_inventory))
        
        ids = self.cursor.execute("SELECT host.fk_scan, host.pk FROM host \
                                   JOIN _host_address ON \
                                   (host.pk = _host_address.fk_host) WHERE \
                                   _host_address.fk_address = (SELECT \
                                   address.pk FROM address WHERE address = ?) \
                                   AND host.fk_scan IN (SELECT fk_scan FROM \
                                   _inventory_scan WHERE fk_inventory = ?) \
                                   ORDER BY host.pk DESC", 
...
new hostid %d @ %s for Inventory id %d" % (old_hid, new_hid, date, 
        ...
        pk = self.cursor.execute("SELECT pk FROM _inventory_changes WHERE \
                                  old_hostid=? AND new_hostid=? AND \
                                  entry_date=? AND \
                                  fk_inventory=?", (old_hid, new_hid, date,
                                                    fk_inventory)).fetchall()
...
Address id %d" % (fk_inventory, fk_address))
        ...
        changes = self.cursor.execute("SELECT old_hostid, new_hostid, \
                        entry_date, short_description, \
                        inventory_change_category.name FROM _inventory_changes\
                        JOIN inventory_change_category ON \
                        (_inventory_changes.fk_category=\
                        inventory_change_category.pk) WHERE fk_inventory=? \
                        AND fk_address=? ORDER BY entry_date DESC",
...
Category id %d" % (fk_inventory, fk_address, fk_category))
        ...
        changes = self.cursor.execute("SELECT old_hostid, new_hostid, \
                        ...
                        WHERE fk_category=? and fk_inventory=? AND \
                        fk_address=? ORDER BY entry_date DESC",
                        (fk_category, fk_inventory, fk_address)).fetchall()

        return changes
    
    
    def get_inventory_changes_for_category_in_range(self, fk_inventory, 
                                                    fk_address, fk_category,
                                                    start, end):
        """
        Returns changes in _inventory_changes for fk_inventory, fk_address and
        ...
        """
        debug("Getting changes for Inventory id %d, Address id %d and \
Category id %d from %s to %s" % (fk_inventory, fk_address, fk_category, start,
                                 end))

        changes = self.cursor.execute("SELECT old_hostid, new_hostid, \
                        entry_date, short_description FROM _inventory_changes\
                        WHERE entry_date > ? AND entry_date < ? AND \
                        fk_category=? AND fk_inventory=? AND \
                        fk_address=? ORDER BY entry_date DESC",
                        (start, end, fk_category, fk_inventory, 
                         fk_address)).fetchall()
